...
Работай самостоятельно, не спрашивая разрешения на каждый шаг:

- Редактировать, создавать и удалять файлы в репозитории.
- Коммитить и пушить в **свою ветку** (не `main`).
- Открывать Pull Request.
- Читать код, запускать `pnpm lint`, `typecheck`, `test`, `build`.

Апрув здесь не нужен: **точка контроля — мёрж PR**, его делает владелец в вебе,
глядя на дифф. Пока изменение не в `main`, оно ничего не ломает.

Спрашивать нужно, только если: правка выходит за рамки поставленной задачи,
задача допускает два принципиально разных решения, или дело касается пунктов из
раздела ниже.
