...
## 5. Ведение задач
Процесс целиком описан в [`process.md`](./process.md): Цели → Проекты → Задачи → Минизадачи.

- Цели — `goals.md`
- Проекты с задачами и минизадачами — `projects/`
- Что делаем в следующий заход — `next-session-agenda.md`
- Входящие идеи — `backlog.md`
- Роадмап (временной срез) — `roadmap.md`

Источник правды — markdown в репе: его читает агент вместе с кодом.
GitHub Issues (шаблоны в `.github/ISSUE_TEMPLATE`) подключим, когда появятся
другие люди или внешние баг-репорты.
